...
if [[ `get_localip` != $host ]];then
    ...
    scp -P 7030 /etc/kubernetes/admin.conf k8s_config setclock_ntp.sh deploy_k8s_m.sh deploy_del_temp.sh ssh-copy-id.sh ssh_trust_init.exp ssh_trust_add.exp root@$host:/root && scp -P 7030 /etc/hosts root@$host:/etc/hosts && ssh -p 7030 root@$host "hostnamectl set-hostname $hostname$num" && ssh -p 7030 root@$host /root/setclock_ntp.sh && ssh -p 7030 root@$host /root/ssh-copy-id.sh && ssh -p 7030 root@$host /root/deploy_k8s_m.sh && ssh -p 7030 root@$host /root/deploy_del_temp.sh
...
scp -P 7030 /etc/kubernetes/admin.conf k8s_config setclock_ntp.sh deploy_k8s_w.sh deploy_del_temp.sh ssh-copy-id.sh ssh_trust_init.exp ssh_trust_add.exp root@$host:/root && scp -P 7030 /etc/hosts root@$host:/etc/hosts && ssh -p 7030 root@$host "hostnamectl set-hostname $hostname_worker$num" && ssh -p 7030 root@$host /root/setclock_ntp.sh && ssh -p 7030 root@$host /root/ssh-copy-id.sh && ssh -p 7030 root@$host /root/deploy_k8s_w.sh && ssh -p 7030 root@$host /root/deploy_del_temp.sh
